    ...
    <div className="min-h-screen grid place-items-center p-4 relative overflow-hidden">
      ...
      <Card className="w-full max-w-md relative animate-fade-in shadow-2xl backdrop-blur-sm bg-white/90 dark:bg-neutral-900/90">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">Create your station</h1>
          <p className="mt-2 text-sm text-neutral-600 dark:text-neutral-400">Get started with your radio station management</p>
        </div>
        
        <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
          <Input label="Station Name" placeholder="Cool FM" {...register("stationName")} error={errors.stationName?.message} />
          <Input label="Frequency" placeholder="96.9" {...register("frequency")} error={errors.frequency?.message} />
          <Input label="Admin Name" placeholder="Admin" {...register("userName")} error={errors.userName?.message} />
          <Input label="Email" type="email" placeholder="admin@coolfm.com" {...register("email")} error={errors.email?.message} />
          <Input label="Password" type="password" placeholder="••••••••" {...register("password")} error={errors.password?.message} />
          <Button className="w-full" type="submit" loading={isLoading}>Create account</Button>
        </form>
        
        <p className="mt-6 text-center text-sm text-neutral-600 dark:text-neutral-400">
          Already have an account?{" "}
          <Link className="font-semibold text-indigo-600 hover:text-indigo-700 dark:text-indigo-400 dark:hover:text-indigo-300 transition-colors" href="/login">
            Sign in
          </Link>
        </p>
      </Card>
